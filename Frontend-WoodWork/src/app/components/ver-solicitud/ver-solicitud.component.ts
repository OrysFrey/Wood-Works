...
export class VerSolicitudComponent {
  ...
  filtrarBudgets():void{
    if(this.amountMin != null && this.amountMax != null && !Number.isNaN(this.amountMin) && !Number.isNaN(this.amountMax)){
      this.id_solicitur = this.activatedRouter.snapshot.params["id"];
      this.budgetService.getBudgets().subscribe({
        next: (data: Presupuesto[]) => {
          data.forEach((budget: Presupuesto) => {
            if(budget.id_solicitud == this.id_solicitur){
              if(budget.monto >= this.amountMin && budget.monto <= this.amountMax){
                this.budgetsAmount.push(budget);
              }
            }
          });
          this.dataSource = new MatTableDataSource(this.budgetsAmount);
          this.budgetsAmount = [];
        },
        error: (err) => {
          console.log(err);
  ...
  clean():void{
    this.cargaMyBudgets();
    const inputElement = document.getElementById('amountMin') as HTMLInputElement;
    inputElement.value = '';
    this.amountMin = NaN;
    const inputElement2 = document.getElementById('amountMax') as HTMLInputElement;
    inputElement2.value = '';
    this.amountMax = NaN;
  }
      ...
      next: (data)=> {
        if( data.id == this.id){
          ...
          if(this.id != 0 && this.id != undefined && this.type == "Carpintero"){
             this.router.navigate(["/dashboard-carpenter/"+this.id+"/solicitudes-carpenter/"+this.id]);
